...
ICudaEngine *createMLPEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt) {
    ...
    fc1->getOutput(0)->setName("out");  // 这行代码是在获取fc1层的第一个输出，并将其名称设置为"out"

    // mark the output
    network->markOutput(*fc1->getOutput(0));  //  这行代码将fc1层的第一个输出标记为网络的输出

    // Set configurations
    builder->setMaxBatchSize(1);   // 这行代码设置了TensorRT构建器（builder）的最大批处理大小为1。
    // Set workspace size
    config->setMaxWorkspaceSize(1 << 20);  // 这行代码设置了TensorRT配置（config）的最大工作区大小为2^20字节，即1MB。
